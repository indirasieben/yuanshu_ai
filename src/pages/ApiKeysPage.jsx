...
function QuotaProgress({ token }) {
  if (token.unlimited_quota)
    return <span className="text-xs text-ink-faint">无限</span>;
  ...
  const expiry =
    token.expired_time === -1
      ...
      : token.expired_time > 0
        ...
        : "—";
  const modelList = token.model_limits
      ...
      <td className="px-3 py-3">
        <div className="flex items-center gap-1">
          ...
          <button
            onClick={() =>
              onCopy(
                revealed && resolvedKey ? `sk-${resolvedKey}` : maskedKey,
                `key-${token.id}`,
              )
            }
            className="p-1 rounded text-ink-faint hover:text-ink bg-transparent border-none cursor-pointer flex-shrink-0"
        ...
        {token.model_limits_enabled && modelList.length > 0 ? (
          <div className="flex flex-wrap gap-1 max-w-[130px]">
            {modelList.slice(0, 2).map((m) => (
              <span
                key={m}
                className="px-1.5 py-0.5 bg-cream-dark text-ink-muted text-[10px] rounded truncate max-w-[60px]"
              >
                {m}
              </span>
            ))}
...
function TokenModal({ mode, token, onClose, onSaved, models, groups }) {
  ...
  const initForm = () => ({
    ...
    group: isCreate ? "default" : token?.group || "default",
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isCreate) {
        ...
        toast.success(
          count > 1 ? `成功创建 ${count} 个 API Key` : "API Key 创建成功",
        );
      } else {
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-medium text-ink">
              {isCreate ? "创建新的 API Key" : "编辑 API Key"}
            </h3>
          </div>
          <button
            onClick={onClose}
      ...
      <div className="bg-card rounded-xl border border-border w-full max-w-sm shadow-lg">
        <div className="px-6 pt-5 pb-3">
          ...
          <p className="text-xs text-ink-muted mt-1">
            请选择复制方式（共 {tokens.length} 个 API Key）
          </p>
        </div>
        ...
        <div className="flex items-start gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-xs text-ink-muted mt-1.5">
              确定要删除所选的 <strong className="text-ink">{count}</strong> 个
              API Key 吗？此操作不可撤销。
            </p>
          </div>
        </div>
        ...
        <div className="bg-card rounded-xl border border-border p-5">
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-ink-muted">
              兼容 OpenAI API 格式，已有代码只需替换{" "}
              <code className="text-ink font-mono bg-cream-dark px-1 py-0.5 rounded text-[11px]">
                base_url
              </code>{" "}
              即可使用。支持 Chatbox、Cursor、Open Interpreter
              等所有兼容客户端。
            </p>
          </div>
        </div>

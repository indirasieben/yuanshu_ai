...
export default function ResetPasswordPage() {
  ...
  const handleSendEmail = async (e) => {
    ...
    setIsLoading(true)
    try {
      // 正确端点：/api/reset_password（不是 /api/verification，后者是注册验证）
      await api.get(`/api/reset_password?email=${encodeURIComponent(email)}`)
      setStep(2)
    } catch (err) {
      toast.error(err.message || '发送失败，请检查邮箱地址是否已注册')
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-cream px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-ink-muted text-sm mt-2">找回密码</p>
        </div>
        ...
        <div className="bg-card rounded-2xl border border-border p-8">
          ...
          {step === 2 && (
            <div className="text-center space-y-4">
              ...
              <p className="text-sm font-medium text-ink">邮件已发送</p>
              <p className="text-xs text-ink-muted leading-relaxed">
                重置链接已发送到 <span className="text-ink font-medium">{email}</span>，
                请查收邮件并点击链接完成密码重置。链接在几分钟内有效。
              </p>
          ...
          <div className="text-center mt-6">
            <Link to="/login" className="text-xs text-ink-muted hover:text-ink no-underline">
              ← 返回登录
            </Link>
          </div>
        </div>

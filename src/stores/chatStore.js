...
import { DEFAULT_MODEL, DEFAULT_CONVERSATION_SETTINGS, MAX_CONVERSATIONS } from '../lib/config'
  ...
  persist(
    (set, get) => ({
      ...
      sendMessage: async (text) => {
        ...
        set({ isStreaming: true, streamingContent: '', streamingReasoningContent: '', abortController })
        ...
        await streamChat({
          ...
          onReasoningChunk: (text) => {
            set(state => ({ streamingReasoningContent: state.streamingReasoningContent + text }))
          },
        ...
        try {
          const title = await chatCompletion({
            ...
            messages: [
              ...
              { role: 'assistant', content: conv.messages[1].content.slice(0, 200) },
              { role: 'user', content: '请用5-10个中文字概括这段对话的主题，只输出标题，不要标点符号' },
            ],
            maxTokens: 30,
          })

          const cleanTitle = title.replace(/["""''、。，！？]/g, '').trim().slice(0, 30)
          if (cleanTitle) {
      ...
      regenerateLastMessage: async () => {
        ...
        await streamChat({
          ...
          onError: (err) => {
            set({ isStreaming: false, streamingContent: '', streamingReasoningContent: '', abortController: null })
          },
        })
      },
    }),
